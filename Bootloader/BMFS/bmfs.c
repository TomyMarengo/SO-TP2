...
int
findfile(char *filename, struct BMFSEntry *fileentry, int *entrynumber) {
    int tint;
...
int
initialize(char *diskname, char *size, char *mbr, char *boot, char *kernel) {
...
static int
StartingBlockCmp(const void *pa, const void *pb) {
...
void
create(char *filename, unsigned long long maxsize) {
