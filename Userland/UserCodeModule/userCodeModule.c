...
static void
help(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static void
divideByZero(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static void
invalidOPCode(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static void
time(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static const char *registerNames[17] = {"RIP", "RAX", "RBX", "RCX", "RDX", "RSI", "RDI", "RBP", "RSP",
                                        "R8 ", "R9 ", "R10", "R11", "R12", "R13", "R14", "R15"};

static void
inforeg(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static char
valueToHexChar(uint8_t value) {
    return value >= 10 ? (value - 10 + 'A') : (value + '0');
}

static void
printMem(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] == 0) {
...
void
divideAndConquer(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static void
fractal(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static void
clear(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) {
    if (parameters[0][0] != 0) {
...
static const char *commands[] = {"clear",   "divideandconquer", "dividebyzero", "fractal", "help",
                                 "inforeg", "invalidopcode",    "printmem",     "time"};
static void (*commands_functions[])(char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS]) = {
    clear, divideAndConquer, divideByZero, fractal, help, inforeg, invalidOPCode, printMem, time};
#define COMMANDS_LENGTH (sizeof(commands) / sizeof(commands[0]))

static int
indexCommand(char *readbuf) {
    char *p = readbuf;
...
void
getCommand(char command[READBUF_LENGTH], char parameters[MAX_PARAMETERS][LENGTH_PARAMETERS], char readbuf[READBUF_LENGTH]) {
    int i, j, k;
