...
Pipe pipeCreate() {
    Pipe pipe = malloc(sizeof(struct PipeData));
    if (pipe == NULL)
        return NULL;
...
int pipeFree(Pipe pipe) {
    return free(pipe->buffer) + free(pipe);
}
...
int pipeAddFd(Pid pid, int fd, Pipe pipe, int allowRead, int allowWrite) {
    ...
    int r = prcAddFd(pid, fd, mapping, allowRead ? &fdReadHandler : NULL, allowWrite ? &fdWriteHandler : NULL, &fdCloseHandler);
    if (r < 0) {
        free(mapping);
        return r;
    }
...
static ssize_t fdReadHandler(Pid pid, int fd, void* resource, char* buf, size_t count) {
    ...
    ssize_t r;
    while ((r = pipeRead(pipe, buf, count)) == 0 && pipe->writerFdCount != 0) {
...
static ssize_t fdWriteHandler(Pid pid, int fd, void* resource, const char* buf, size_t count) {
    ...
    // lockPipe();
    ssize_t r;
    while ((r = pipeWrite(pipe, buf, count)) == 0 && pipe->readerFdCount != 0) {
        // Add to queue of blocked
        // unlockPipe();
        // sch_block(pid);
        // sch_ready(pidOtro);
