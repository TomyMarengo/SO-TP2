...
static ssize_t readHandler(int fd, char* buffer, size_t count) {
    return handleReadFdProcess(getpid(), fd, buffer, count);
}

static ssize_t writeHandler(int fd, const char* buffer, size_t count) {
    return handleWriteFdProcess(getpid(), fd, buffer, count);
}

static int closeHandler(int fd) {
    return deleteFdProcess(getpid(), fd);
}

static int clearScreenHandler() {
...
static unsigned long millisHandler() {
    return TICKS_TO_MILLISECONDS(getElapsedTicks());
}

static int timeHandler(char* buffer) {
    getCurrentTimeString(buffer);
    return 0;
}

static int dateHandler(char* buffer) {
...
static int freeHandler(void* ptr) {
    return handleFree(getpid(), ptr);
}

static void* reallocHandler(void* ptr, size_t size) {
    return handleRealloc(getpid(), ptr, size);
}

static int memoryStateHandler(MemoryState* memoryState) {
    return getStateMemory(memoryState);
}

static Pid getpidHandler() {
    return getpid();
}

static Pid createProcessHandler(int stdinMapFd, int stdoutMapFd, int stderrMapFd, const ProcessCreateInfo* createInfo) {
...
static int exitHandler() {
    kill(getpid());
    yield();
    return 1;
}

static int blockHandler(Pid pid) {
...
static int unblockHandler(Pid pid) {
    return unblock(pid);
}
...
static int killHandler(Pid pid) {
    int result = kill(pid);
    if (pid == getpid())
        yield();
    return result;
...
static int priorityHandler(Pid pid, Priority priority) {
    return setPriority(pid, priority);
}

static int listProcessesHandler(ProcessInfo* array, int maxProcesses) {
    return listProcesses(array, maxProcesses);
}

static int waitpidHandler(Pid pid) {
...
static int createPipeHandler(int pipefd[2]) {
    Pid pid = getpid();

    Pipe pipe;
    int readFd = -1, writeFd = -1;

    if ((pipe = createPipe()) < 0 || (readFd = addFdPipe(pid, -1, pipe, 1, 0)) < 0 || (writeFd = addFdPipe(pid, -1, pipe, 0, 1)) < 0) {
        if (readFd >= 0)
...
static int openPipeHandler(const char* name, int pipefd[2]) {
    Pid pid = getpid();

    Pipe pipe;
    int readFd = -1, writeFd = -1;

    if ((pipe = openPipe(name)) < 0 || (readFd = addFdPipe(pid, -1, pipe, 1, 0)) < 0 || (writeFd = addFdPipe(pid, -1, pipe, 0, 1)) < 0) {
        if (readFd >= 0)
            deleteFdProcess(pid, readFd);
        if (pipe >= 0)
            freePipe(pipe);
...
static int unlinkPipeHandler(const char* name) {
    return unlinkPipe(name);
}

static int listPipesHandler(PipeInfo* array, int maxPipes) {
    return listPipes(array, maxPipes);
}

static Sem openSemHandler(const char* name, unsigned int value) {
    return openSem(name, value);
}

static int closeSemHandler(Sem sem) {
    return closeSem(sem);
}

static int postSemHandler(Sem sem) {
    return post(sem);
}

static int waitSemHandler(Sem sem) {
    return wait(sem);
}

static int listSemaphoresHandler(SemaphoreInfo* array, int maxSemaphores) {
